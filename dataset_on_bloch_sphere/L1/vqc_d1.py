...
pi = np.pi
x1 = np.random.uniform(-pi / 2, 0, 400)

upper_right = np.array([x1])

x2 = np.random.uniform(0, pi / 2, 400)

upper_left = np.array([x2])

x3 = np.random.uniform(pi / 2, pi, 400)

down_left = np.array([x3])

x4 = np.random.uniform(-pi, -pi / 2, 400)

down_right = np.array([x4])

X = np.concatenate((upper_right.T, upper_left.T, down_left.T, down_right.T), axis=0)
Y = np.concatenate((np.ones(400), np.zeros(400), np.zeros(400), np.ones(400)), axis=0)
