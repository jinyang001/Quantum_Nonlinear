...
pi = np.pi
shift=pi/4
x1 = np.random.uniform(-pi / 2, 0-shift, 800)

upper_right = np.array([x1])

x2 = np.random.uniform(0-shift, pi / 2, 800)

upper_left = np.array([x2])

x3 = np.random.uniform(pi / 2, pi-shift, 800)

down_left = np.array([x3])

x4 = np.random.uniform(-pi-shift, -pi / 2, 800)
...
test_loader2 = DataLoader(dataset=test_dataset2, batch_size=300, shuffle=False, num_workers=0, drop_last=True)
