...
class QFCModel(tq.QuantumModule):
    class QLayer(tq.QuantumModule):
        ...
        def forward(self, q_device: tq.QuantumDevice):
            ...
            for q in range(0, self.n_wires, 4):
                ...
                crx_index += 1
                getattr(self, 'crx' + str(crx_index))(self.q_device, wires=[q + 3, q])
                crx_index += 1
            for q in range(0, self.n_wires, 2):
                getattr(self, 'rx' + str(rx_index))(self.q_device, wires=q)
                getattr(self, 'ry' + str(ry_index))(self.q_device, wires=q + 1)
            ...
            for q in range(7):
                getattr(self, 'crx' + str(crx_index))(self.q_device, wires=[q, q + 1])
                crx_index += 1
            getattr(self, 'crx' + str(crx_index))(self.q_device, wires=[7, 0])
            crx_index += 1
    ...
    def forward(self, x):
        ...
        # ss
        state = out2
        state = state.view([out2.shape[0]] + [2] * self.q_device.n_wires)
