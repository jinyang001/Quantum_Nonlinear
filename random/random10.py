    ...
    def forward(self, x, use_qiskit=False):
        ...
        if use_qiskit:
            ...
            for k in range(bsz):
                ...
                state = x[k]
                block1 = torch.stack((state[0], state[1], state[4], state[5],state[8], state[9], state[12], state[13]))

                block2 = torch.stack((state[2], state[3], state[6], state[7],state[10], state[11], state[14], state[15]))

                chunks1 = [block1, block2]
                ...
                circ.initialize(chunks1[0], [0, 1,2])
                circ.initialize(chunks1[1], [3, 4,5])
            ...
            for i, t in enumerate(out):
                ...
                block1 = torch.stack((t[0], t[1], t[4], t[5], t[8], t[9], t[12], t[13]))

                block2 = torch.stack(
                    (t[2], t[3], t[6], t[7], t[10], t[11], t[14], t[15]))
        ...
        x = x.reshape(bsz, 2, 3).sum(-1).squeeze()
        # print(x.shape)
        # print(x)
        # sss
