...
pi = np.pi
x1 = np.random.uniform(-pi / 2, 0, 300)

upper_right = np.array([x1])

x2 = np.random.uniform(0, pi / 2, 100)

upper_left = np.array([x2])

x3 = np.random.uniform(pi / 2, pi, 100)

down_left = np.array([x3])

x4 = np.random.uniform(-pi, -pi / 2, 100)

down_right = np.array([x4])

X = np.concatenate((upper_right.T, upper_left.T, down_left.T, down_right.T), axis=0)
Y = np.concatenate((np.ones(300), np.zeros(100), np.zeros(100), np.zeros(100)), axis=0)
...
def train(dataloader, model, criterion, optimizer, device):
    ...
    for _, (X, y) in enumerate(dataloader):
        ...
        #print(outputs)
        pred=torch.where(outputs>=0.62, 1, 0)
