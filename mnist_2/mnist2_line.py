    ...
    def forward(self, x, use_qiskit=False):
        ...
        if use_qiskit:
            ...
            for k in range(bsz):
                ...
                state = x[k]
                block1 = torch.stack((state[0], state[1], state[2], state[3]))
                block2 = torch.stack((state[4], state[5], state[6], state[7]))
                block3 = torch.stack((state[8], state[9], state[10], state[11]))
                block4 = torch.stack((state[12], state[13], state[14], state[15]))
            ...
            for i, t in enumerate(out):
                ...
                block1 = torch.stack((t[0], t[1], t[2], t[3]))
                block2 = torch.stack((t[4], t[5], t[6], t[7]))
                block3 = torch.stack((t[8], t[9], t[10], t[11]))
                block4 = torch.stack((t[12], t[13], t[14], t[15]))
        ...
        x = x.reshape(bsz, 2, 4).sum(-1).squeeze()
        # print(x.shape)
        # print(x)
        # sss
